...
namespace Dissonance.Networking.Client
{
        ...
        where TPeer : struct
    {
        ...
        public TextReceiver(EventQueue events, Rooms rooms, IClientCollection<TPeer?> peers)
        {
            if (events == null)
        ...
        {
            //Parse packet
            var txt = reader.ReadTextPacket(true);
        ...
        [CanBeNull]private string GetTxtMessageRecipient(ChannelType txtRecipientType, ushort txtRecipient)
        {
            if (txtRecipientType == ChannelType.Player)
            {
                ...
                throw new ArgumentOutOfRangeException("txtRecipientType");
        }
    }
}
